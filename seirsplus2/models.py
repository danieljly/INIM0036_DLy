...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def update_parameters(self):
        ...
        self.p_local = numpy.array(self.parameters['p_local']).reshape(
            (self.numNodes, 1)) if isinstance(
                self.parameters['p_local'], (list, numpy.ndarray)) else numpy.full(
                    fill_value=self.parameters['p_local'], shape=(self.numNodes, 1))
                
        self.p_global = numpy.array(self.parameters['p_global']).reshape(
            (self.numNodes, 1)) if isinstance(
                self.parameters['p_global'], (list, numpy.ndarray)) else numpy.full(
                    fill_value=self.parameters['p_global'], shape=(self.numNodes, 1))
